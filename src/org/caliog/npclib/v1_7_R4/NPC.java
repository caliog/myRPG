...
public class NPC extends org.caliog.npclib.NPC {
	private net.minecraft.server.v1_7_R4.ItemStack[] previousEquipment = { null, null, null, null, null };
	...
	public void updateEquipment() {
		...
		int changes = 0;
		net.minecraft.server.v1_7_R4.ItemStack[] newI = new net.minecraft.server.v1_7_R4.ItemStack[previousEquipment.length];
		for (int i = 0; i < previousEquipment.length; i++) {
			net.minecraft.server.v1_7_R4.ItemStack previous = previousEquipment[i];
			net.minecraft.server.v1_7_R4.ItemStack current = ((EntityPlayer) getEntity()).getEquipment(i);
			newI[i] = current;
			if (current == null) {
				if (previous != null) {
				...
				if (!net.minecraft.server.v1_7_R4.ItemStack.equals(previous, current) || (previous != null && !previous.equals(current))) {
					NPCUtils.sendPacketNearby(getBukkitEntity().getLocation(),
							new PacketPlayOutEntityEquipment(getEntity().getId(), i, current));
					++changes;
				}
