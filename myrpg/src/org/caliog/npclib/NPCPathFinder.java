...
public class NPCPathFinder extends Thread {
	...
	while (c != endNode && rep < max) { // Repetition variable prevents infinite loop when destination is unreachable
		...
		for (int j = -1; j <= 1; j++) {
			...
			if (adjacentBlock != c
				&& !(j == 1 && adjacentBlock.b.getRelative(0, -1, 0).getType() == Material.FENCE)) {
	...
	if (node.xPos != parent.xPos && node.zPos != parent.zPos) {
	    ...
	    final boolean xZCor1 = !getNode(parent.b.getRelative(0, 0, zDir)).isNotsolid();
	    final boolean xZCor2 = !getNode(parent.b.getRelative(xDir, 0, 0)).isNotsolid();

	    corner = xZCor1 || xZCor2;
	} else if (node.xPos != parent.xPos && node.yPos != parent.yPos || node.yPos != parent.yPos
		&& node.zPos != parent.zPos) {
	    corner = node.yPos > parent.yPos ? !getNode(parent.b.getRelative(0, 2, 0)).isNotsolid() : !getNode(node.b
		    .getRelative(0, 2, 0)).isNotsolid();
	}
	...
	return !corner
		&& (node.isNotsolid() && (!nodeBelow.isNotsolid() || nodeBelow.isLiquid() && node.isLiquid()) && nodeAbove.isNotsolid() || node == endNode);
    }

    private void scoreBlock(Node node, Node parent) {
	final int diagonal = node.xPos != parent.xPos && node.zPos != parent.zPos || node.xPos != parent.xPos
		&& node.yPos != parent.yPos || node.yPos != parent.yPos && node.zPos != parent.zPos ? 14 : 10;
